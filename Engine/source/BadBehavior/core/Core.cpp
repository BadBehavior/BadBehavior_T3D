...
void EventListenerNode::initPersistFields()
{
   addGroup( "Behavior" );
   
   addField( "event", TypeRealString, Offset(mEventName, EventListenerNode),
      "@brief The name of the event to handle." );
...
void EventListenerNode::handleEvent(EventListenerTask *task)
{
   task->onEvent();
}

const String& EventListenerNode::getEventName() 
{ 
   return mEventName; 
}
...
EventListenerTask::EventListenerTask(Node &node, SimObject &owner, BehaviorTreeRunner &runner) 
   : Parent(node, owner, runner), 
     mEventReceived(false) 
{
}

void EventListenerTask::onInitialize()
{
   Parent::onInitialize();
   mEventReceived = false;
   mRunner->subscribeToEvent(static_cast<EventListenerNode*>(mNodeRep)->getEventName().c_str(), this);
}

void EventListenerTask::onEvent()
{
   mEventReceived = true;
   mRunner->unsubscribeFromEvent(static_cast<EventListenerNode*>(mNodeRep)->getEventName().c_str(), this);
}